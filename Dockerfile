...
RUN export GOARCH=${TARGETPLATFORM#*/} && if [ "$GOARCH" = "arm64" ]; then export CC=aarch64-linux-gnu-gcc && export CXX=aarch64-linux-gnu-cpp && export AR=aarch64-linux-gnu-ar;fi && bash build.sh $VERSION $COMMIT_SHA
