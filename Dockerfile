...
WORKDIR /build
COPY . .
RUN apt-get update && apt-get install -y libre2-dev && if [ "$TARGETPLATFORM" == "linux/arm64"] ; then apt-get install -y crossbuild-essential-aarch64;else apt-get install -y build-essential; fi
