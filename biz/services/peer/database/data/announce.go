...
func (r *PeerRepository) PickPeers(ctx context.Context, infoHashRaw string, numWant int) ([]*entity.Peers, error) {
	...
	err := r.db.WithContext(ctx).Where("info_hash = ? AND last_seen > ? AND event != ?", infoHash, validTime, common.PeerEvent_Stopped).Order("RAND()").Limit(numWant).Find(&peers).Error
	if err != nil {
		return nil, err
	}
	return peers, nil
